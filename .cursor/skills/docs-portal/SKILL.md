...
3. Append `(docs_dir, site_name)` to `LEAVES` in `tools/buildtools/docs.py`.
4. Add a product card on the matching topology hub page under `docs/landing/`:
...
5. Verify with `poe docs` and open `site/index.html` → topology card → product card.
